...
<ul id="slider-{$slides[0].slideshowid}" class="slideshow">
	{foreach from=$slides item=s}<li style="width:{$s.width}px;height:{$s.height}px;">
	{if $s.titles && $s.title}<div class="slidetitle">{$s.title}</div>{/if}
	{if $s.image}{if $s.imagelink}<a href="{$s.imagelink}" rel="lightbox">{/if}<img src="{$SITEURL}/images/{$s.imagewidth}x{$s.imageheight}/{$s.image}" width="{$s.imagewidth}" height="{$s.imageheight}" title="{$s.title}" alt="" />{if $s.imagelink}</a>{/if}
